...
def plot_class_distribution(
    ...
    try:
        plt.figure(figsize=(6, 4))
        sns.countplot(x=y, palette="viridis", dodge=False)
...
def plot_feature_importance(
    ...
    """Plot and save feature importance without redundant hue."""
    try:
...
def plot_feature_correlation_heatmap(
    ...
    try:
        ...
        if logger:
            logger.error(
                f"Error plotting feature correlation heatmap: {str(e)}", exc_info=True
            )
        raise
    finally:
        plt.close()
...
def plot_missing_values(
    ...
    """
    Create a heatmap visualization of missing values in the dataset with percentage annotations.
    """
    try:
        ...
        missing_indices = np.where(data.isnull())
        ...
        # Annotate percentage missing per feature
        for i, column in enumerate(data.columns):
            plt.text(
                i + 0.5,
                -0.5,
                f"{missing_percentage[column]:.1f}%",
                ha="center",
                va="top",
                fontsize=9,
                color="black",
        ...
        plt.tight_layout()
        filename = f"{model_name.replace(' ', '_')}_missing_values_heatmap.png"
        save_plot(plt, report_dir, filename, logger)

    except Exception as e:
        if logger:
            logger.error(
                f"Error plotting missing values heatmap: {str(e)}", exc_info=True
            )
        raise
    finally:
        plt.close()
...
def plot_temporal_progression(
    ...
    try:
        ...
        for patient_id in selected_patients:
            ...
            patient_data = patient_data.sort_values(by="Hour")  # Ensure data is sorted
...
def plot_error_analysis(
    ...
    try:
        ...
        if logger:
            logger.error(f"Error plotting error analysis: {str(e)}", exc_info=True)
        raise
    finally:
        plt.close("all")
...
def plot_calibration(
    ...
    try:
        ...
        if logger:
            logger.error(f"Error plotting calibration curve: {str(e)}", exc_info=True)
        raise
    finally:
        plt.close()
...
def plot_prediction_timeline(
    ...
    model_name: str = "model",
    max_patients: int = 5,
    threshold: float = 0.5,  # Added threshold parameter
    logger: Optional[logging.Logger] = None,
) -> None:
    """
    ...
    try:
        ...
        for patient_id in selected_patients:
            ...
            pred_onset_indices = np.where(patient_preds > threshold)[0]
        ...
        if logger:
            logger.error(f"Error plotting prediction timeline: {str(e)}", exc_info=True)
        raise
    finally:
        plt.close("all")
